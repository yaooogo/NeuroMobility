...
<template>
  <ManageLayout title="投资订单" description="查看用户投资、等待期与分红进度。">
    <section class="panel-card">
      ...
      <div class="table-wrap"><table class="data-table" style="min-width: 1780px">
        <thead><tr><th>ID</th><th>订单号</th><th>钱包</th><th>投资金额</th><th>已分红</th><th>总分红</th><th>等待期</th><th>分红周期</th><th>分红范围</th><th>保底分红</th><th>整车</th><th>状态</th><th>等待截止</th><th>下次分红</th><th>投资时间</th></tr></thead>
        <tbody>
          <tr v-if="loading || !list.length"><td colspan="15" class="empty-cell">{{ loading ? '正在加载...' : '暂无投资订单' }}</td></tr>
          <tr v-for="row in list" :key="row.id"><td>{{ row.id }}</td><td class="mono-cell">{{ row.order_id }}</td><td class="mono-cell">{{ row.wallet }}</td><td>{{ formatFixedAmount(row.amount) }} U</td><td>{{ formatFixedAmount(row.distributed_amount) }} U</td><td>{{ formatFixedAmount(row.total_dividend) }} U</td><td>{{ row.waiting_days }}天</td><td>{{ row.cycle_days }}天</td><td>{{ row.min_percent }}% ~ {{ row.max_percent }}%</td><td>{{ row.guaranteed_percent }}%</td><td>{{ row.whole_vehicle === 1 ? '是' : '否' }}</td><td>{{ statusLabels[row.status] || '未知' }}</td><td>{{ row.waiting_until || '-' }}</td><td>{{ row.next_dividend_at || '-' }}</td><td>{{ row.created_at || '-' }}</td></tr>
        </tbody>
      </table></div>
