...
const success = ref('');
const form = reactive({ id: 0, title: '', cover: '', content: '', sort: 0, status: 1 });
...
function openCreate() {
  Object.assign(form, { id: 0, title: '', cover: '', content: '', sort: 0, status: 1 });
  modalError.value = ''; modal.value = true;
}
function edit(row) {
  Object.assign(form, { id: row.id, title: row.title, cover: row.cover, content: row.content, sort: row.sort, status: row.status });
  modalError.value = ''; modal.value = true;
}
  ...
  <ManageLayout title="公告管理" description="管理公告封面、标题、内容、排序和状态。">
    <section class="panel-card">
      ...
      <div class="table-wrap"><table class="data-table" style="min-width: 1150px">
        <thead><tr><th>ID</th><th>封面</th><th>标题</th><th>内容</th><th>排序</th><th>状态</th><th>创建时间</th><th>更新时间</th><th v-if="can(user, 'announcements-update') || can(user, 'announcements-delete')">操作</th></tr></thead>
        <tbody>
          <tr v-if="loading || !list.length"><td :colspan="can(user, 'announcements-update') || can(user, 'announcements-delete') ? 9 : 8" class="empty-cell">{{ loading ? '正在加载...' : '暂无公告' }}</td></tr>
          <tr v-for="row in list" :key="row.id"><td>{{ row.id }}</td><td><img v-if="row.cover" class="announcement-cover" :src="row.cover" alt="公告封面" /><span v-else>-</span></td><td><strong>{{ row.title }}</strong></td><td class="announcement-content">{{ row.content }}</td><td>{{ row.sort }}</td><td><span class="status-badge" :class="row.status === 1 ? 'status-badge--on' : 'status-badge--off'">{{ row.status === 1 ? '启用' : '禁用' }}</span></td><td>{{ row.created_at || '-' }}</td><td>{{ row.updated_at || '-' }}</td><td v-if="can(user, 'announcements-update') || can(user, 'announcements-delete')" class="actions-cell"><button v-if="can(user, 'announcements-update')" class="table-button" @click="edit(row)">编辑</button><button v-if="can(user, 'announcements-delete')" class="table-button table-button--danger" :disabled="deleting === row.id" @click="remove(row)">删除</button></td></tr>
        </tbody>
      </table></div>
